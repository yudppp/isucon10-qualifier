...
func NewMySQLConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		Host:     getEnv("MYSQL_HOST", "10.162.73.103"),
...
func initialize(c echo.Context) error {
	...
	paths := []string{
		...
		filepath.Join(sqlDir, "3_DummyFeatures.sql"),
		filepath.Join(sqlDir, "4_AddGeo.sql"),
	}
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	var estates []Estate
	size := []int64{chair.Width, chair.Height, chair.Depth}
	...
	err = db.SelectContext(ctx, &estates, query, size[2], size[1], size[1], size[2], Limit)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.JSON(http.StatusOK, EstateListResponse{[]Estate{}})
		}
